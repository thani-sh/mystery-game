...
async function removeBackgroundFromBuffer(pixelArray, channels) {
...
async function removeBackground(inputPath, outputPath) {
  try {
    const image = import_sharp.default(inputPath);
    ...
    if (metadata.hasAlpha) {
      ...
      await import_sharp.default(processedPixels, {
        raw: {
          width: info.width,
          height: info.height,
          channels: info.channels
        }
      }).png({ compressionLevel: 9, effort: 10 }).toFile(outputPath);
    }
  } catch (error) {
...
async function processAsset(options) {
  const { sourcePath, targetPath, removeBackground: shouldRemoveBg } = options;
  await mkdir(dirname(targetPath), { recursive: true });
  if (shouldRemoveBg) {
    console.log(`  \uD83D\uDCF8 Processing with background removal and optimization: ${relative(ASSETS_DIR, sourcePath)}`);
    ...
    await import_sharp.default(sourcePath).png({ compressionLevel: 9, effort: 10 }).toFile(targetPath);
  } else {
...
async function processDirectory(sourceDir, targetDir) {
  const entries = await Array.fromAsync(new Bun.Glob("**/*").scan({ cwd: sourceDir, onlyFiles: false }));
